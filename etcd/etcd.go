		...
		case service := <-e.serviceCh:
			...
			if ok {
				...
				for _, node := range service.Instances {
					registryService.service.Instances = append(registryService.service.Instances, node)
				}
...
func (e *EtcdRegistry) registerService(registryService *RegisterService) (err error) {
	...
	for _, node := range registryService.service.Instances {
...
func (e *EtcdRegistry) serviceNodePath(service *registry.Service) string {
	...
	nodeIP := fmt.Sprintf("%s:%d", service.Instances[0].IP, service.Instances[0].Port)
...
func (e *EtcdRegistry) GetService(ctx context.Context,
	...
	for _, kv := range resp.Kvs {
		...
		for _, node := range tmpService.Instances {
			service.Instances = append(service.Instances, node)
		}
	}
...
func (e *EtcdRegistry) syncServiceFromEtcd() {
	...
	for _, service := range allServiceInfo.serviceMap {
		...
		for _, kv := range resp.Kvs {
			...
			for _, node := range tmpService.Instances {
				serviceNew.Instances = append(serviceNew.Instances, node)
			}
		}
