...
func (p *PluginMgr) registerPlugin(plugin IRegistry) (err error) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *PluginMgr) initRegistry(ctx context.Context, name string,
	opts ...Option) (registry IRegistry, err error) {
	//查找对应的插件是否存在
	p.lock.Lock()
	defer p.lock.Unlock()
...
// 注册插件
func RegisterPlugin(registry IRegistry) (err error) {
	return pluginMgr.registerPlugin(registry)
}

// 初始化注册中心
func InitRegistry(ctx context.Context, name string, opts ...Option) (registry IRegistry, err error) {
